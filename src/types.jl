...
macro syms(xs...)
    defs = map(xs) do x
        nt = _name_type(x)
        n, t = nt.name, nt.type
        m = get(nt, :metadata, Dict{Type,Any}())
...
arraymeta(foo) = Dict{Type, Any}(ArrayCtx => foo)
...
function _name_type(x)
    ...
    elseif x isa Expr && x.head === :ref
        ...
        return (name=ntype.name,
                ...
                metadata=:(arraymeta(ArrayShape(Base.Slice.(($(x.args[2:end]...),))))))
...
Term{T}(f, args) where {T} = Term{rec_promote_symtype(f, map(symtype, args)...)}(f, args, Dict{Type,Any}())
